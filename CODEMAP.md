...
- `pages/AlertDetailPage.tsx` — детальная карточка алерта (описание, похожие алерты, действия); рядом с «Индекс» показывает значение `event.type` из ECS-документа (или `type`, если `event.type` отсутствует).
- `pages/CasePage.tsx` — страница инцидента: timeline/граф событий, IOC, отчёт, участники; для присоединённых инцидентов подтягивает и объединяет таймлайн/граф/IOC дочерних инцидентов (аналогично алертам).
- `pages/AnalysisPage.tsx` — страница «Анализ»: поиск + граф корреляций между алертами и сущностями (обёртка над `CorrelationGraph`).
- `pages/StatisticsPage.tsx` — графики и агрегированная статистика по алертам за период: по статусам/типу угрозы/URL/IP/учёткам/файлам, с текстовым поиском по этим таблицам; вкладки «Обзор»/«Отчёт» (см. `StatisticsReportPanel`), период и поиск общие для обеих.
...
- `components/Statistics/StatisticsReportPanel.tsx` — подраздел «Отчёт» в статистике: авто-сводка для SOC-аналитика, одна колонка шириной 3/4 экрана — подробная таблица по типам угроз (доля, полоса распределения), топ URL/IP/учёток/файлов с учётом того же поиска, что и в «Обзоре», динамика по времени; экспорт в PDF (jsPDF/html2canvas).
- `components/Cases/IOCPanel.tsx` — список и добавление IOC инцидента; если есть присоединённые инциденты, дополнительно подтягивает их IOC и показывает столбец «Присоединён» со ссылкой на дочерний инцидент (удаление недоступно для чужих IOC).
