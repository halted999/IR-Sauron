...
- `components/Statistics/StatisticsReportPanel.tsx` — подраздел «Отчёт» в статистике: авто-сводка для SOC-аналитика (текст, статусы, типы угроз, топ URL/IP/учёток/файлов с учётом того же поиска, что и в «Обзоре», динамика по времени), экспорт в PDF (jsPDF/html2canvas).
